...
function Home() {
  ...
  const fetchNews = async () => {
    ...
    const cloudinaryResponse = await axios.get("http://localhost:6001/cloudinary/get-latest-images");
    if (cloudinaryResponse.data.images) {
      setTopThreeNews((prevNews) => {
        return prevNews.map((news, index) => ({
          ...news,
          image: cloudinaryResponse.data.images[index]?.url || news.image, // Replace with Cloudinary image
        }));
      });
    }
